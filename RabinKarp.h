...
void createBaseList(int w, int h, string *dataHeight, string *dataWidth, long long *&baseList, long long **&hashHeight, long long **&hashWidth)
{
	...
	for (int i = 1; i <= max(w, h); i++)
		baseList[i] = (baseList[i - 1] * 256) % MOD;

	// creata hash table height
	hashHeight = new long long *[w];
	for (int i = 0; i < w; i++)
		hashHeight[i] = new long long[dataHeight[i].length()];
	for (int i = 0; i < w; i++)
	{
		hashHeight[i][0] = 0;
		for (int j = 1; j < dataHeight[i].length(); j++)
			hashHeight[i][j] = (hashHeight[i][j - 1] * 256 + int(dataHeight[i][j])) % MOD;
	}

	// create hash table width
	hashWidth = new long long *[h];
	for (int i = 0; i < h; i++)
		hashWidth[i] = new long long[dataWidth[i].length()];
	for (int i = 0; i < h; i++)
	{
		hashWidth[i][0] = 0;
		for (int j = 1; j < dataWidth[i].length(); j++)
			hashWidth[i][j] = (hashWidth[i][j - 1] * 256 + int(dataWidth[i][j])) % MOD;
	}
}
...
long long getHash(string valueString)
{
	...
	for (int i = 1; i < valueString.length(); i++)
		hashValue = (hashValue * 256 + int(valueString[i])) % MOD;
...
{
	if (findString.length() > lengthDataString)
		return 0;
	const long long MOD = 1e9 + 7;
	for (int i = 1; i <= lengthDataString - findString.length() + 1; i++)
	{
		...
		if (hashFindString == valueHashFromIToEnd)
			return i;
	}
